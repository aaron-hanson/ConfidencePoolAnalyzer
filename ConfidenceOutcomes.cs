...
namespace ConfidenceOutcomes
{
    class ConfidenceOutcomes
    {
        ...
        static void ComplementSearch()
        {
            int numGames = Matchups.Count();
            long max = (long)Math.Pow(2, numGames) - 1;
            List<PlayerEntry> testEntries = new List<PlayerEntry>();

            for (int bits = 0; bits <= max; bits++)
            {
                PlayerEntry testEntry = new PlayerEntry("Teresa Mendoz", bits, null);
                testEntry.SetProbability();
                testEntries.Add(testEntry);
            }

            //List<PlayerEntry> topEntries = testEntries.OrderBy(x => 0 - x.probability).Take(8).ToList();
            foreach (PlayerEntry entry in testEntries.OrderByDescending(x => x.Probability)) 
            {
                PlayerEntries.Add(entry);

                int startPoints = 16;
                entry.GamePicks.ForEach(x => x.Points = startPoints--);
                double bestProb = -1;
                string bestEntry = "";
                bool madeMove = true;
                while (madeMove)
                {
                    Console.Write("/");
                    madeMove = false;
                    foreach (GamePick pick in entry.GamePicks.OrderByDescending(x => x.Points))
                    {
                        Console.Write(".");
                        for (int tryPoints = 16; tryPoints >= 17 - Matchups.Count(); tryPoints--)
                        {
                            double origPoints = pick.Points;
                            entry.GamePicks.Where(x => !x.Equals(pick) && tryPoints >= x.Points && origPoints < x.Points).ToList().ForEach(x => x.Points--);
                            entry.GamePicks.Where(x => !x.Equals(pick) && tryPoints <= x.Points && origPoints > x.Points).ToList().ForEach(x => x.Points++);
                            pick.Points = tryPoints;

                            CalculateOutcomes();
                            double overallWinProb = 100 * Possibilities.Where(x => x.PlayerScores.Count(y => EntryWinCheck.Contains(y.Name) && y.Rank == 1) > 0)
                                                                        .Sum(x => x.Probability * (double)x.PlayerScores.Count(y => EntryWinCheck.Contains(y.Name) && y.Rank == 1) / (double)x.PlayerScores.Count(y => y.Rank == 1));
                            if (overallWinProb > bestProb)
                            {
                                madeMove = true;
                                bestProb = overallWinProb;
                                bestEntry = entry.ToString();
                            }
                            else
                            {
                                pick.Points = origPoints;
                                entry.GamePicks.Where(x => !x.Equals(pick) && tryPoints < x.Points && origPoints >= x.Points).ToList().ForEach(x => x.Points--);
                                entry.GamePicks.Where(x => !x.Equals(pick) && tryPoints > x.Points && origPoints <= x.Points).ToList().ForEach(x => x.Points++);
                            }
                        }
                ...
                Console.WriteLine("\n" + Math.Round(bestProb, 4).ToString(CultureInfo.InvariantCulture).PadRight(9) + "\t" + bestEntry);
        ...
        static void TeresaSearch()
        {
            PlayerEntry teresaEntry = PlayerEntries.First(x => x.Name.Equals("Teresa Mendoz"));

            int startPoints = 16;
            teresaEntry.GamePicks.ForEach(x => x.Points = startPoints--);
            double bestProb = -1;
            string bestEntry = "";
            bool madeMove = true;
            while (madeMove)
            {
                Console.Write("/");
                madeMove = false;
                foreach (GamePick pick in teresaEntry.GamePicks.OrderByDescending(x => x.Points))
                {
                    Console.Write(".");
                    for (int tryPoints = 16; tryPoints >= 17 - Matchups.Count(); tryPoints--)
                    {
                        double origPoints = pick.Points;
                        teresaEntry.GamePicks.Where(x => !x.Equals(pick) && tryPoints >= x.Points && origPoints < x.Points).ToList().ForEach(x => x.Points--);
                        teresaEntry.GamePicks.Where(x => !x.Equals(pick) && tryPoints <= x.Points && origPoints > x.Points).ToList().ForEach(x => x.Points++);
                        pick.Points = tryPoints;
                        ...
                        if (overallWinProb > bestProb)
                        {
                            madeMove = true;
                            bestProb = overallWinProb;
                            bestEntry = teresaEntry.ToString();
                        }
                        else
                        {
                            pick.Points = origPoints;
